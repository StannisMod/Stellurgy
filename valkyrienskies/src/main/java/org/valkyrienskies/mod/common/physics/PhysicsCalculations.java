...
public class PhysicsCalculations {
    ...
    private boolean isPhysicsBroken() {
        if (getAngularVelocity().lengthSquared() > 50000 || getLinearVelocity().lengthSquared() > 50000 || !getAngularVelocity().isFinite() || !getLinearVelocity().isFinite()) {
                ...
                if (blockAt instanceof IBlockForceProvider) {
                    try {
                        ...
                        if (otherPosition != null) {
                            ...
                            inBodyWO.set(mutablePos.getX() + 0.5, mutablePos.getY() + 0.5, mutablePos.getZ() + 0.5);
                            inBodyWO.sub(physCenterOfMass);
                            getParent().getShipTransformationManager().getCurrentPhysicsTransform().transformDirection(inBodyWO, TransformType.SUBSPACE_TO_GLOBAL);
                        }
    ...
    public double getDragForPhysTick() {
        return Math.pow(DRAG_CONSTANT, getPhysicsTimeDeltaPerPhysTick() * 20.0);
    }
    ...
    @java.lang.SuppressWarnings("all")
    public void setForceToUseGameTransform(final boolean forceToUseGameTransform) {
