...
/**
 * The DURABLE facts this mod holds about one player: what he is aboard, and two windows he is
 ...
 *
 * <h2>Why only THREE things are here</h2>
 *
 ...
 * it EXCLUDES two of the five things the mod holds per player:</p>
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Both remain live state owned by their own subsystems, and a release still lets go of them —
...
public interface IPlayerBindings {
    ...
    /**
     * Consecutive ticks adrift in hyperspace, or {@code 0}.
     *
     * <p><b>DECLARED AND NOT YET USED, on purpose.</b> The argument for keeping the count here is
     * that losing it on a relog lets a crew member the void is counting down to killing reset it by
     * disconnecting. The argument against predates this interface and is written at
     * {@code HyperspaceVoid.pruneDeparted}: a returning player is placed by the login restore, which
     * is *a fresh judgement, not a continuation*, so resuming a countdown asserts something about a
     * situation he may no longer be in. That reasoning was found mid-migration, after the field had
     * been planned; it is the one the code was built on and it stands until it is overturned
     * deliberately, so the counter still lives in {@code HyperspaceVoid}'s own map.</p>
     *
     * <p>The accessors stay so the decision has somewhere to land, and so that whoever takes it
     * finds this paragraph rather than an empty space.</p>
     */
    int adriftTicks();

    void setAdriftTicks(int ticks);
